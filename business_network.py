...
def visualize_net(net,figfile="business_net.pdf",cutoffs=None, maxcols=6, disp_dict=None):
	...
	connected_components = [net.subgraph(ccomp) for ccomp in nx.connected_components(net.to_undirected())]
	organized_components = arrange_components(connected_components,cutoffs,maxcols)
	...
	for subplot,(subrows,subcols,connected_components) in zip(subplots,organized_components):
		...
		for idx,ccomp in enumerate(connected_components):
			disp_dict["node_color"] = [(random(),random(),random())] * ccomp.order()
...
def ND_lines_to_net(jl_filename="ND_businesses.jl",entity_attrs=None):
	if entity_attrs == None:
		entity_attrs = ["Owner Name","Commercial Registered Agent","Owners","Registered Agent"]
	...
	with open(jl_filename,"rU") as jl_file:
		for ln in jl_file:
			...
			parents = set((biz_dict[attr].split("\n")[0].lower() for attr in entity_attrs if attr in biz_dict)) - {''}
			if name not in net:
				net.add_node(name)
